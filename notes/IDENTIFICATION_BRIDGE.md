...
In-paper anchor: `paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of nontrivial invariant subspaces for admissible bounded operators through an admissible spectral closure architecture`.
...
- endpoint lock: `subspace_transfer`.
...
2. the endpoint-transfer lock `subspace_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_ISP` remains positive.
...
Payload: verify `subspace_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_ISP` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `subspace_transfer`
